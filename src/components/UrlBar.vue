...
import { getStoredUrls, getUiState, saveUiState, type RecentEntry } from '@/utils/storage'
import { deriveSourceName, getSourceColor } from '@/services/swaggerMultiLoader'
...
<template>
  <header class="glass-surface sticky top-0 z-30">
    ...
    <div v-if="collapsed" class="flex items-center justify-between px-3 py-1.5">
      <span class="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500">{{ t('urlBar.toolbar') }}</span>
      <button
        type="button"
    ...
    <div v-else class="flex flex-col gap-3 px-3 py-3">
      <div class="flex flex-wrap items-center gap-2">
        ...
        <div class="relative min-w-0 flex-1">
          ...
          >
            ...
            >
              <span class="truncate font-medium text-slate-200">{{ entry.name || deriveSourceName(entry.url) }}</span>
        ...
        <button
          ...
          @click="emit('add-source', 'Swagger Petstore (示例)', 'example://petstore')"
        >
        ...
        <div class="relative w-64 shrink-0">
          <AppIcon name="search" :size="14" class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
          <input
            :value="searchInput"
            type="text"
        ...
        <div class="relative shrink-0">
          <button
            ...
            @click="showToken = false; showSettings = !showSettings"
          >
          ...
          >
            <p class="mb-1.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">{{ t('settings.language') }}</p>
            ...
            <p class="mb-1.5 mt-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">{{ t('theme.title') }}</p>
            <div class="grid gap-1">
              <button
                ...
                :class="props.themeMode === option.mode ? 'sidebar-item-active' : ''"
                @click="switchTheme(option.mode)"
              >
                <span>{{ t(option.key) }}</span>
                <AppIcon v-if="props.themeMode === option.mode" name="check" :size="14" class="text-sky-400" />
              </button>
            </div>
          </div>
        </div>
        ...
        <div class="relative shrink-0">
          <button
            type="button"
            class="toolbar-button h-9 w-9 px-0"
            :class="tokenActive ? 'border-emerald-400/30 text-emerald-300' : (showToken ? 'border-sky-400/30 text-slate-100' : '')"
            :title="t('urlBar.tokenSettings')"
            @click="showSettings = false; showToken = !showToken"
          >
          ...
          >
            <p class="mb-1.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">{{ t('common.token') }}</p>
            <input
              :value="token"
              type="text"
        ...
        <button
          ...
          :class="showDiff ? 'border-amber-400/20 bg-amber-400/[0.08] text-amber-200' : ''"
          @click="emit('toggle-diff')"
        >
          Diff
          <span class="rounded-md bg-white/10 px-1.5 py-0.5 text-[11px]">{{ diffCount }}</span>
        </button>
      ...
      <div v-if="sources.length > 0" class="flex items-center gap-2 overflow-auto pb-0.5">
        <span class="shrink-0 text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500">
        ...
        <button
          ...
          :class="selectedSource === '__ALL__'
            ? 'border-sky-400/20 bg-sky-400/[0.08] text-slate-100'
            : 'text-slate-400 hover:text-slate-100'"
          @click="emit('select-source', '__ALL__')"
        >
          {{ t('common.all') }}
          <span class="text-slate-500">{{ sources.reduce((sum, source) => sum + source.apis.length, 0) }}</span>
        </button>
        ...
        <div
          ...
          :class="selectedSource === source.id
            ...
            : [getSourceColor(index).text, getSourceColor(index).bgHover, getSourceColor(index).borderHover]"
          :title="source.url"
        >
          <button
            type="button"
    ...
    >
      <div class="popover-surface w-full max-w-md p-5">
        <h3 class="text-sm font-semibold text-slate-100">{{ t('urlBar.addSourceTitle') }}</h3>
        <p class="mt-1 break-all text-xs text-slate-500">{{ urlInput }}</p>

        <div class="mt-4">
          <label class="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">{{ t('urlBar.sourceName') }}</label>
          <input
            ref="nameInputEl"
            v-model="pendingName"
            type="text"
          ...
          />
          <p class="mt-1.5 text-xs text-slate-500">{{ t('urlBar.sourceNameHint') }}</p>
        </div>
