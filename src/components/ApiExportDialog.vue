...
import { generateApiContext, exportApiKey, type ExportFormat } from '@/core/aiContextGenerator'
...
watch(
  ...
  (open) => {
    ...
    const initialKeys = new Set((props.initialSelected || []).map(exportApiKey))
...
<template>
  ...
  >
    <div class="popover-surface flex h-full max-h-[88vh] w-full max-w-6xl flex-col overflow-hidden">
      <div class="flex items-start justify-between border-b border-white/10 px-5 py-4">
        <div>
          <h3 class="text-sm font-semibold text-slate-100">{{ t('aiExport.title') }}</h3>
          <p class="mt-1 text-xs text-slate-400">{{ t('aiExport.description') }}</p>
        </div>
        <button
          type="button"
      ...
      <div class="grid min-h-0 flex-1 grid-cols-[minmax(320px,2fr)_minmax(0,3fr)]">
        <div class="flex min-h-0 flex-col border-r border-white/10">
          ...
          <div class="min-h-0 flex-1 overflow-auto px-2 py-2">
            <div v-if="groups.length === 0" class="px-3 py-6 text-center text-sm text-slate-500">
              {{ t('explorer.empty') }}
            </div>

            <section v-for="group in groups" :key="group.tag" class="mb-1">
              <div class="flex items-center gap-1 rounded-lg px-1 py-1 transition hover:bg-white/[0.03]">
                <button
                  type="button"
                  ...
                  :title="isTagExpanded(group.tag) ? t('common.hide') : t('common.show')"
                ...
                <label class="flex min-w-0 flex-1 cursor-pointer items-center gap-2 py-1">
                  ...
                  />
                  <span class="flex-1 truncate text-xs font-semibold uppercase tracking-[0.12em] text-slate-300">
                    {{ group.tag }}
                  </span>
                  <span class="rounded-md bg-white/5 px-1.5 py-0.5 text-[10px] text-slate-500">
                    {{ group.apis.length }}
                  </span>
                </label>
              </div>
              ...
              >
                ...
                >
                  ...
                  <span class="min-w-0 flex-1 truncate font-mono text-xs text-slate-200" :title="api.path">
                    {{ api.path }}
                  </span>
                  <span
                    v-if="api.summary"
